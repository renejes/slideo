...
Die **Feature-Roadmap §18/§19 ist im Wesentlichen abgeschlossen** (Kurzrecap unten). Es geht jetzt **nicht mehr
um neue Features**, sondern um **„verifizieren & polieren"**. Konkreter Plan für die nächste Session:

1. **Vollständiger GUI-Test (ZUERST, der eigentliche Fokus der nächsten Session).** Bisher ist alles nur
   automatisch grün (`cargo test` 27, `typecheck`, `vite build` + Multi-Agent-Reviews je Feature), aber **noch
   nichts in der echten App durchgeklickt**. Wir gehen die Checklisten **A1–A7 in `docs/next-steps.md`
   Punkt für Punkt durch** — der **Mensch testet** in `npm run tauri:dev` (frisch gebaut; danach **Claude
   Desktop neu starten** wegen der 35 MCP-Tools), die **KI fixt bestätigte Findings**. Besonders prüfen:
   Zweitfenster auf echter **Multi-Display**-Hardware, die neuen MCP-Tools (`set_logo`/`clear_logo`/`register_font`/
   `set_presentation_title`/`set_zone_label`), Auto-Animate, Komponenten-Palette.
2. **Danach: Polish anschauen.** Gemeinsam aus den geparkten Kandidaten auswählen, was sich lohnt (Liste in
   `docs/next-steps.md` → „Polish-Kandidaten"): u.a. bild-basierter PPTX-Export (Browser-Offload),
   Komponenten-Set erweitern, Laser/Stift aufs Zweitfenster spiegeln, flackerfreies Projector-Update,
   Asset-Positionierung „Large", Font-Subset, Asset-Library-Politur.

Später (eigener Schritt, Abschnitt C in next-steps.md): **Distribution & Notarization** (Signing, notarisierte
Builds) — erst sinnvoll, wenn GUI-Test + Polish durch sind.
...
Bitte bestätige kurz, dass du die vier Dokumente gelesen hast, fasse den Stand in 3–4 Sätzen in eigenen Worten zusammen, und **lass uns dann mit dem vollständigen GUI-Test starten**: geh die Checkliste A1–A7 in `docs/next-steps.md` mit mir Punkt für Punkt durch (ich teste in der laufenden App, du fixt bestätigte Findings). Danach schauen wir gemeinsam, welcher Polish sich lohnt.
